...
data class NetworkStatsInternal(var uid: Int, var name: String?, var packageName: String?, var icon: Drawable?, var valueDownload: Long, var valueUpload: Long) {
    ...
    fun formatValueNoPoint(value: Long?): String {
        ...
        if (tera.compareTo(1.0) > 0) {
            return "${App.context().getString(R.string.network_value_format_no_point).format(tera)} ${App.context().getString(R.string.unit_tera)}"
        } else if (giga.compareTo(1.0) > 0) {
            return "${App.context().getString(R.string.network_value_format_no_point).format(giga)} ${App.context().getString(R.string.unit_giga)}"
        } else if (mega.compareTo(1.0) > 0) {
            return "${App.context().getString(R.string.network_value_format_no_point).format(mega)} ${App.context().getString(R.string.unit_mega)}"
        } else {
            return "${App.context().getString(R.string.network_value_format_no_point).format(kilo)} ${App.context().getString(R.string.unit_kilo)}"
        }
    }
}